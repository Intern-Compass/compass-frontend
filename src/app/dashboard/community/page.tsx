...
export default function CommunityPage() {
  ...
  const posts = [
    { 
      ...
      content: "I'm working on a mobile app project and would love to hear about best practices for React Native development. What are some common pitfalls to avoid?" 
    },
    { 
      id: 2, 
  ...
  const replies = {
    1: [
      { 
        ...
        position: "Supervisor",
        content: "Great question! I'd recommend focusing on performance optimization early. Use FlatList for large datasets and avoid unnecessary re-renders." 
      },
      { 
        id: 1, 
        author: "Jane Doe", 
    ...
    2: [
      { 
        ...
        author: "David Smith", 
        position: "Supervisor",
        content: "Great question! I'd recommend focusing on performance optimization early." 
      },
      { 
        id: 1, 
        author: "Mary Johnson", 
  ...
  <div className="p-8 md:px-16 md:py-8 space-y-8">
    ...
    <div className="rounded-2xl flex flex-col">
      <figure>
        <div className={ show404 || (!show404 && filteredPosts.length === 0)
                  ...
                  {filteredPosts.map(post => (
                    <div key={post.id} className="p-4 w-full md:w-10/11 mb-8">
                      <div className="flex items-center space-x-4 mb-2">
                        ...
                        <div>
                          <h1>{post.author}</h1>
                          <p className="text-muted-foreground text-sm font-normal">{post.position}, {post.division}</p>
                        </div>
                      </div>
                      <div className="space-y-2">
                        ...
                        <div className="flex space-x-3 text-muted-foreground text-xs">
                          <ThumbsUp 
                            className={`w-3 h-3 cursor-pointer ${likedPostId === post.id ? "text-chart-1" : ""}`}
                          ...
                          <span
                            ...
                            onClick={() =>
                              setOpenRepliesId(openRepliesId === post.id ? null : post.id)
                            }
                          >
                            2 replies
                          </span>
                            ...
                            {replies[post.id]?.map(reply => (
                              <div key={reply.id} className="mb-2">
                                <div className="border border-primary p-4 rounded-lg ">
                                  <h1>{reply.author}</h1>
                                  <p className="text-muted-foreground text-sm font-normalmb-2 md:mb-4">{reply.position}</p>
                                  <p className="text-xs">{reply.content}</p>
                                </div>
                              </div>
                            ))}
                            <div className="flex items-center space-x-2 mt-4">
                              <input type="text" placeholder="Leave a comment" className="border border-muted text-sm w-full px-2 py-4 rounded-lg"/>
        ...
        <figcaption className="text-center">
          {show404 ? (
            <h2 className="font-medium text-muted-foreground">No search found</h2>
          ):(
          filteredPosts.length === 0 ? (
          ...
          {!show404 && filteredPosts.length === 0 && (
            <button className="bg-primary py-2 px-4 text-sm font-medium rounded-3xl cursor-pointer">Join the community</button>
          )}
        </figcaption>
      </figure>
    </div>
